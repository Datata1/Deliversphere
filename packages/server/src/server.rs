...
// --- KORREKTUR 1: Bereinigte Imports ---
// Nur noch die Typen, die wir wirklich brauchen
...
use runner::{
    ...
    runner_service_server::RunnerService,
...
async fn broadcast_ws_message(clients: &WsClientMap, message: &WsServerMessage) {
    ...
    for entry in clients.iter() {
        ...
        if tx.send(message.clone()).is_err() {
            println!("Failed to send WS message to client {}, will be cleaned up on next disconnect.", entry.key());
        }
    }
}
...
impl RunnerService for MyRunnerService {
    ...
    async fn communicate(
        ...
        let (tx, rx) = mpsc::channel(128); // Dieser Channel ist für gRPC-Befehle AN den Agenten

        let output_stream = tokio_stream::wrappers::ReceiverStream::new(rx);
        let response = Response::new(Box::pin(output_stream) as Self::CommunicateStream);

        // Klone die Pools/Maps für den asynchronen Task
        ...
        tokio::spawn(async move {
            ...
            if let Some(Ok(first_msg)) = inbound.next().await {
                if let Some(Payload::Register(reg)) = first_msg.payload {
                    ...
                    agent_id = Some(reg.agent_id.clone()); // Klonen, da 'reg' noch gebraucht wird
                    ...
                    // SQL-Query: Agent in DB eintragen oder aktualisieren
                    ...
                    // --- KORREKTUR 2: Bereinigter Broadcast-Block ---
                    // Hole den aktualisierten Agenten aus der DB für die WS-Nachricht
                    ...
                    if let Ok(Some(agent)) = agent_result {
                        ...
                        eprintln!("Konnte Agent {} nach Registrierung nicht aus DB laden.", reg.agent_id);
                    }

                    // Optional: gRPC-Sender in LiveAgentMap speichern
                    live_agents.insert(reg.agent_id, tx.clone());
                    println!("Agent '{}' ist jetzt online (Registrierung abgeschlossen).", agent_id.as_ref().unwrap());
            ...
            // Sicherstellen, dass wir eine agent_id haben
            let current_agent_id = match agent_id {
                Some(id) => id,
                None => return,
            };

            // 2. In der Schleife auf Heartbeats/Logs lauschen
            ...
            // 3. Cleanup nach Verbindungsabbruch
            ...
            // Hole den Agenten erneut, um den 'offline'-Status zu senden
            let agent_result = sqlx::query_as::<_, Agent>("SELECT * FROM agents WHERE id = ?")
                .bind(&current_agent_id)
                .fetch_optional(&db_pool)
                .await;
        ...
        }); // Ende des tokio::spawn

        Ok(response)
    } // --- KORREKTUR 3: Semikolon entfernt ---
}